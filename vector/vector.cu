...
__device__ __host__ vec2f vec2f::operator+(vec2f s) const {
...
__device__ __host__ vec2f vec2f::operator*(const float s) const {
...
__device__ __host__ float vec2f::dot(const vec2f s) const {
    return x * s.x + y * s.y;
}

__device__ __host__ float vec2f::magnitude() const {
    return std::sqrt(dot(*this));
}

__device__ __host__ vec2f vec2f::normalize() const {
...
__device__ __host__ vec3f vec3f::operator*(const vec3f s) const {
    return vec3f{ x * s.x, y * s.y, z * s.z };
}
